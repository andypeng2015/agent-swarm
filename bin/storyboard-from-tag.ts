 ...
 * drives the `ReleaseShell` Remotion composition (assets/video-source/) and the
 * release recorder. `demo_script_id` and `vo_line` are left blank for a human /
 * the recorder to fill in.
 *
 * Usage:
 *   bun bin/storyboard-from-tag.ts <newTag> [<oldTag>] [--out <path>] [--repo <owner/name>]
 *
 *   bun bin/storyboard-from-tag.ts v1.80.2
 *       → diff v1.80.2 against the previous tag
 *   bun bin/storyboard-from-tag.ts v1.80.2 v1.79.4
 *       → diff the explicit v1.79.4..v1.80.2 range
 *   bun bin/storyboard-from-tag.ts v1.80.2 --out storyboard.json
