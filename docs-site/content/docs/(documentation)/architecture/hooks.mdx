...
- **Identity sync** — If an identity file (SOUL.md, IDENTITY.md, TOOLS.md, CLAUDE.md) was edited, syncs the changes to the database with `changeSource: "self_edit"` tracking to distinguish agent edits from session-end syncs. The session-start runner now records baseline hashes for those files; the Stop/session-end sync path skips unchanged files so lead-side `update-profile` edits made during the session are not overwritten by stale local copies. Files shorter than 100 characters are rejected to prevent accidental profile corruption
