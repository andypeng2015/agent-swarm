...
| `HEARTBEAT_RESUME_PIN_GRACE_MIN` | `10` | Grace window (minutes) a same-agent-pinned crash-recovery resume waits before the reaper escalates it to a Lead re-delegation decision. Set to `0` to disable the reaper |
...
These can be supplied as process env vars or managed as swarm config values (via `set-config` with `scope: "global"`). The no-config default is `local-fs`; set `AGENT_FS_API_URL` to keep shared agent-fs behavior. The API boot seeder registers a service identity with agent-fs on first boot, stores generated bootstrap and per-agent API keys in encrypted swarm config, creates the shared org/drive, and persists `AGENT_FS_DEFAULT_*`. The two `AGENT_FS_DEFAULT_*` keys let the server fill in attachment IDs server-side so the renderer can emit full live-host URLs — per-row IDs always win, and missing config + missing row IDs still falls back to the `agent-fs:<path>` form.
