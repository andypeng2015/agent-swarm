...
| `SCRIPT_RUNTIME_DIR` | — | Directory holding the pre-built script-runtime bundles (`eval-harness.bundle.js`, `stdlib.bundle.js`, `swarm-sdk.bundle.js`). Set automatically in the Docker image (`/app/scripts-runtime`). Required by the scripts runtime when running as a compiled binary, since the harness subprocess cannot read the binary's `/$bunfs/` virtual filesystem. |
...
| `CLAUDE_BINARY` | No | `claude` | Argv prefix for the Claude CLI. Accepts a single binary name (`claude`, `shannon`), an absolute path, or a whitespace-separated command string (`bunx @dexh/shannon`, `npx -y @dexh/shannon`). Setting it to anything containing `shannon` opts into the [Shannon (experimental)](/docs/guides/shannon-experimental) variant — fail-fasts at session-create if `tmux` is missing, pre-seeds `~/.claude.json` to skip the trust dialog. Reloadable via `swarm_config` (precedence: repo > agent > global > env > `claude`) so you can flip a worker via `set-config CLAUDE_BINARY=...` without a container restart. |
...
OpenTelemetry traces are disabled unless `OTEL_EXPORTER_OTLP_ENDPOINT` is set. See [Observability with OpenTelemetry](/docs/guides/observability-opentelemetry) for SigNoz setup, emitted spans, and query examples.
...
| `OTEL_TRACE_POLL` | OFF | When set to `1`/`true`, emits trace spans for worker poll loop (`worker.poll`) and the `/api/poll` HTTP endpoint. Skip by default to reduce span volume. |
