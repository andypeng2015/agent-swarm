...
| `TRUST_BODY_REQUESTED_BY_USER_ID` | `false` (unset/off) | Set to `true` only in single-tenant/shared-operator-key deployments where every `POST /api/tasks` caller is already trusted. When enabled, the server may honor a body-supplied `requestedByUserId` (validated against a real user row) if no authenticated/owned-task identity is available. Leave it off in multi-tenant or upstream-safe deployments to preserve the default anti-spoofing behavior. |
...
| `MEMORY_HYBRID_SEARCH` | `0` | Set to `1` to enable hybrid memory retrieval that blends vector similarity with a full-text pass before reranking. Leave unset or `0` for vector-only search. |
| `MEMORY_GRAPH_EXPANSION` | `0` | Set to `1` to expand search candidates with 1-hop memory-link neighbors (resolved `[[wikilink]]` targets) before reranking. Neighbors are damped (`parentRawSimilarity × strength × 0.7`), capped at 5 per search, respect the caller's scope/source/lead ACL, and carry `retrievalSource: "graph"` so `GET /api/memory/usefulness` can measure the arm's citation rate. Default off — results are byte-identical to pre-expansion behavior. |
...
Environment variables for optional [Archil](https://www.archil.com/) FUSE disk mounts, giving worker containers persistent shared and per-agent personal disks. Skipped entirely when `ARCHIL_MOUNT_TOKEN` is unset (local dev / non-Archil environments).

| Variable | Description |
|----------|-------------|
| `ARCHIL_MOUNT_TOKEN` | Auth token for the Archil CLI. Gates the entire feature — unset disables disk mounting |
| `ARCHIL_REGION` | Archil region for the mount (e.g. `aws-eu-west-1`) |
| `ARCHIL_SHARED_DISK_NAME` | Shared disk identifier, mounted at `/workspace/shared` |
| `ARCHIL_PERSONAL_DISK_NAME` | Per-agent personal disk identifier, mounted at `/workspace/personal` |
...
| Variable | Description |
|----------|-------------|
| `PAGE_SESSION_SECRET` | Optional HMAC-SHA256 secret for signing the `page_session` cookie. Falls back to the swarm API key (`getApiKey()`) when unset; boot-time signing/verification refuses to run if both are unset |
