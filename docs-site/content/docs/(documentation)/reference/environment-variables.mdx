...
| `SWARM_DEP_POSTGRES_ENABLED` | No | `false` | Set to `true` to start the optional bundled PostgreSQL 16 cluster on worker boot (entrypoint runs `scripts/init-local-postgres.sh`). Binaries ship dormant in the worker image since `v1.93.0` |
