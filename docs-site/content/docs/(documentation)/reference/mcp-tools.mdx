...
Use `script_run` for swarm-visible, durable scripts. For local-only throwaway TS, use code-mode `run`.
...
Return the generated `.d.ts` SDK surface (derived from the MCP tool registry via `scripts/bundle-script-types.ts`). Useful for type-checking scripts locally before `script_upsert`.
