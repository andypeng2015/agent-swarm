...
substitution rules without exposing raw secrets to scripts.

### script-connections

Lead-only registry for typed script API connections. Phase 1 supports
OpenAPI-backed `ctx.api.<slug>` clients plus optional linkage to a credential
binding for authenticated requests.
...
Manage recurring and one-time task automation. Any registered agent can update or delete schedules. Schedules support both a concrete `model` override and a portable `modelTier` (`smol`, `regular`, `smart`, `ultra`) that is passed through to the tasks they create.
