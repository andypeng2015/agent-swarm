...
├─ 👀 Review dependencies — awaiting reviewer feedback
└─ ⏳ Write report
```

Status icons cover the full task lifecycle: 🗂️ `backlog`, 📭 `unassigned`, 📨 `offered`, 👀 `reviewing`, 📡 `pending`, ⏳ `in_progress`, ⏸️ `paused`, ✅ `completed`, ❌ `failed`, 🚫 `cancelled`, and ↪️ `superseded`. Trees show up to 8 children per node with an overflow indicator for larger task graphs.
...
For delegated tasks tracked in a task tree, substantive prose output is posted inline in the originating thread when the agent has not already used `slack-reply` and the task has no primary attachment. Primary file, page, and pull-request deliverables keep the compact completion card because its task link already points at the deliverable. Inline prose is converted to Slack mrkdwn and capped at 4,000 characters; truncated messages end with a link to the full task output.

The watcher also persists its tracked task-message map across restarts, so an API or worker reboot does not recreate already-seen Slack thread updates as fresh tasks.
