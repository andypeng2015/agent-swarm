...
| --------- | ------------------------------------- | ---------------------------------------- |
| `openapi` | `ctx.api.<slug>.<operationId>(args)`  | a spec URL or inline JSON spec           |
...
The spec is fetched, parsed (JSON specs only in v1), and each operation becomes
...
"you can close this tab" page. Then bind the token:
...
- JSON OpenAPI specs only (no YAML).
