...
description: Group tasks, workflows, schedules, pages, and provider-backed files under canonical shared or personal namespace keys
---
...
When a workflow creates a task, the task inherits the workflow key unless the task node supplies one. Scheduler-created tasks inherit the schedule key. Other create paths default to `shared/`.
