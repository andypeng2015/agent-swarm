...
Fresh swarms also ship with an expanded built-in global script catalog. Today
that includes 18 seeded helpers, including task-onboarding utilities like
`task-context-gathering` and `smart-recall`, ops audits like
`schedule-health`, `task-failure-audit`, `compound-insights`, and
`boot-triage`, and memory-quality reporting via `memory-eval`.
