...
<Callout type="warn">
  **Shannon is deprecated.** Use Desplega's `@desplega.ai/claude-bridge` via `SWARM_USE_CLAUDE_BRIDGE=true`. Existing `CLAUDE_BINARY=shannon` setups still work for compatibility, but new deployments should not use shannon.
</Callout>
...
Claude Bridge owns the prompt pre-clear flow. It updates Claude's global project trust/onboarding entries, writes per-workdir `.claude/settings.local.json` for dangerous-mode bypass, launches Claude with `--dangerously-skip-permissions`, and auto-accepts supported startup prompts from the tmux pane.

Because that behavior lives in claude-bridge, the swarm does **not** run the legacy shannon trust pre-seed when `SWARM_USE_CLAUDE_BRIDGE=true`.

## Shannon compatibility

Existing shannon deployments continue to work:
...
`CLAUDE_BINARY` is still whitespace-split into argv tokens and is still reloadable through `swarm_config`. When the resolved value contains `shannon`, the adapter:

- emits a deprecation warning pointing to `SWARM_USE_CLAUDE_BRIDGE=true`
- checks that `tmux` is on `PATH`
- runs the legacy `$HOME/.claude.json` trust pre-seed before spawning
...
See also: [`runbooks/harness-providers.md`](https://github.com/desplega-ai/agent-swarm/blob/main/runbooks/harness-providers.md) — engineering reference for the bridge and deprecated shannon wiring.
