...
- **Rate-limit-aware selection**: the runner queries `/api/keys/available?keyType=CODEX_OAUTH` before each task spawn and picks from non-rate-limited slots only. When a task hits a rate limit, the slot is marked unavailable for ~5 minutes (until structured `rate_limit_event` capture lands — tracked separately).
