...
Source of truth: [`packages/harness/src/types.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/types.ts).
...
| File | Transport | Auth |
|---|---|---|
| [`packages/harness/src/claude-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/claude-adapter.ts) | `Bun.spawn` of `claude` CLI with `--output-format stream-json`, JSONL stdout parsing | `CLAUDE_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY` |
| [`packages/harness/src/pi-mono-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/pi-mono-adapter.ts) | In-process via `createAgentSession` from `@earendil-works/pi-coding-agent`; no subprocess | `ANTHROPIC_API_KEY` / `OPENROUTER_API_KEY` / `~/.pi/agent/auth.json` — or, when `BEDROCK_AUTH_MODE=sdk` or `MODEL_OVERRIDE=amazon-bedrock/*`, AWS SDK default chain (probed via `ListFoundationModels`; see [pi-mono + Amazon Bedrock](#pi-mono--amazon-bedrock-auth) below) |
| [`packages/harness/src/codex-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/codex-adapter.ts) | Parent adapter spawns `src/commands/codex-session-runner.ts`, sends session config over stdin, and receives line-delimited events/results over stdout; the child hosts `@openai/codex-sdk` `Codex` / `Thread` and streams `ThreadEvent`s back | `OPENAI_API_KEY` or ChatGPT OAuth stored in `swarm_config.codex_oauth` (see §6) |
| [`packages/harness/src/claude-managed-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/claude-managed-adapter.ts) | Anthropic SDK `client.beta.sessions.events.stream` (SSE); session executes in Anthropic's managed cloud sandbox, worker is a thin relay | `ANTHROPIC_API_KEY` plus pre-existing `MANAGED_AGENT_ID` + `MANAGED_ENVIRONMENT_ID` from one-time `claude-managed-setup` CLI (see §13) |
...
All four are wired together by the factory at [`packages/harness/src/index.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/index.ts):
...
- **Harness-agnostic, runner-driven.** Because the sync runs in the runner — at the single point where every completed session converges, including crashes — it does **not** depend on the harness emitting a shutdown event. This is what makes it reliable for `codex`/`opencode` (which previously had no sync path) and for `pi` (whose in-extension `session_shutdown` sync could silently not-fire). The Claude plugin Stop hook ([`src/hooks/hook.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/src/hooks/hook.ts)) and the pi extension ([`packages/harness/src/pi-mono-extension.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/pi-mono-extension.ts)) still run their own sync; the runner-level call is the authoritative backstop.
...
- **Claude** ([`claude-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/claude-adapter.ts)):
...
- **Pi-mono** ([`pi-mono-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/pi-mono-adapter.ts)):
  - **Every** normalized `ProviderEvent` is written as `{...event, timestamp}` inside `emit()` (L167–187). Closed in `runSession`'s `finally` at L327.
- **Codex** ([`codex-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/codex-adapter.ts)):
...
- UI hook: `useTaskSessionLogs` in [`apps/ui/src/api/hooks/use-tasks.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/apps/ui/src/api/hooks/use-tasks.ts), consumed at `apps/ui/src/pages/tasks/[id]/page.tsx:42` and rendered by `<SessionLogViewer />` at `apps/ui/src/components/shared/session-log-viewer.tsx`.
...
|---|---|---|
| **Claude** | Discovers an existing `.mcp.json` (walking up from `cwd`), injects `X-Source-Task-Id` into the `agent-swarm` entry, writes a per-session copy to `/tmp/mcp-<taskId>.json`, launches CLI with `--mcp-config <path> --strict-mcp-config`. | [`claude-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/claude-adapter.ts) L117–184, L251 |
| **Pi** | Constructs `McpHttpClient(apiUrl, apiKey, agentId, taskId)`, calls `listTools()`, wraps each as a pi-mono `ToolDefinition` with prefix `mcp__<name>__`. | [`pi-mono-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/pi-mono-adapter.ts) L410–421, [`pi-mono-mcp-client.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/pi-mono-mcp-client.ts) L53 |
| **Codex** | `buildCodexConfig` registers `mcp_servers["agent-swarm"] = { url: "{apiUrl}/mcp", http_headers: { Authorization, X-Agent-ID, X-Source-Task-Id }, bearer_token_env_var, startup_timeout_sec }`. Passed to `new Codex({ config })`. | [`codex-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/codex-adapter.ts) L132–243, L857–861 |
...
- Pi: `try/catch` around discovery ([`pi-mono-adapter.ts` L409–424](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/pi-mono-adapter.ts)) — on failure, `customTools = []`.
...
Even if the in-process MCP connection fails, the **runner** and the adapter's own swarm-event hooks still talk to the swarm HTTP API directly for lifecycle, heartbeat, and cancellation polling — see [`packages/harness/src/codex-swarm-events.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/codex-swarm-events.ts) and [`packages/harness/src/pi-mono-extension.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/pi-mono-extension.ts) L1–50. That is the safety net; MCP is what lets the *model* call tools.
...
- **Claude** — CLI flag: `cmd.push("--append-system-prompt", this.config.systemPrompt)` at [`claude-adapter.ts:245–247`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/claude-adapter.ts).
- **Codex** — written into `AGENTS.md` in `cwd` inside a `<swarm_system_prompt>` block. The Codex SDK has no `--append-system-prompt` equivalent; this file is the only channel. Entry: `writeCodexAgentsMd(config.cwd, config.systemPrompt)` at [`codex-adapter.ts:761`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/codex-adapter.ts); implementation at [`codex-agents-md.ts:56–119`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/codex-agents-md.ts). If an `AGENTS.md` exists, the block is prepended; otherwise stacked atop any `CLAUDE.md`. Cleanup in the session `finally` at `codex-adapter.ts:738`.
- **Pi** — SDK parameter: `new DefaultResourceLoader({ appendSystemPrompt: [config.systemPrompt], ... })` passed via `CreateAgentSessionOptions` at [`pi-mono-adapter.ts:508–519`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/pi-mono-adapter.ts).
...
<Tabs items={["Native slash-commands", "Prefixed slash-commands", "Inline resolver"]}>
  <Tab value="Native slash-commands">
    **Claude.** The CLI already knows about skills installed under `~/.claude/skills/<name>/SKILL.md`. The adapter just returns `/<name>`.
    ...
    See [`claude-adapter.ts:601`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/claude-adapter.ts).
  </Tab>
  <Tab value="Prefixed slash-commands">
    **Pi.** Pi-mono supports skills but namespaces them:
    ...
    See [`pi-mono-adapter.ts:541`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/pi-mono-adapter.ts). Skills live at `~/.pi/agent/skills/<name>/SKILL.md`.
  </Tab>
  <Tab value="Inline resolver">
    **Codex and OpenCode.** These SDKs have no `SKILL.md` mechanism, so the adapter resolves the skill itself before calling the model:
    ...
    - [`packages/harness/src/codex-skill-resolver.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/codex-skill-resolver.ts) intercepts a leading `/<name>` in the user prompt.
...
Create `packages/harness/src/foo-adapter.ts`:
...
Edit [`packages/harness/src/index.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/index.ts):
...
**Reference code:**
- Claude JSONL branching: [`packages/harness/src/claude-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/claude-adapter.ts) around L368–470.
- Codex `ThreadEvent` dispatcher: [`packages/harness/src/codex-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/codex-adapter.ts) around L463–618.
- Pi `AgentSessionEvent` dispatcher: [`packages/harness/src/pi-mono-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/pi-mono-adapter.ts) around L161+.
...
If your SDK returns tokens but not USD, compute cost from a pricing table (see [`packages/harness/src/codex-models.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/codex-models.ts) for the Codex model/pricing resolver pattern).
...
- Do you accept shortnames (`"sonnet"`, `"gpt-5"`) and expand to full IDs? If so, build a resolver — see `resolveCodexModel` in [`packages/harness/src/codex-models.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/codex-models.ts) and `resolveModel` in [`packages/harness/src/pi-mono-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/pi-mono-adapter.ts).
...
<Tabs items={["Env-var only", "OAuth in swarm_config", "File-based"]}>
  <Tab value="Env-var only">
    Claude-style. Validate at adapter start; throw a clear error if missing. Example: `validateClaudeCredentials()` in [`packages/harness/src/claude-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/claude-adapter.ts).
  </Tab>
...
- Codex: [`packages/harness/src/codex-swarm-events.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/codex-swarm-events.ts) — throttled `fireAndForget` fetches for cancel/heartbeat/activity/context-usage, attached via `this.listeners.push(...)` inside the session.
- Pi: [`packages/harness/src/pi-mono-extension.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/pi-mono-extension.ts) — `createSwarmHooksExtension` passed into `DefaultResourceLoader({ extensionFactories: [swarmExtension] })`.
...
- **Codex**: no native skills support → [`packages/harness/src/codex-skill-resolver.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/codex-skill-resolver.ts) intercepts a leading `/<name>` in the prompt, reads `${CODEX_SKILLS_DIR ?? ~/.codex/skills}/<name>/SKILL.md`, and inlines it before calling `thread.runStreamed`. The system prompt is delivered by writing `AGENTS.md` into `cwd` (see [`codex-agents-md.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/codex-agents-md.ts)).
...
1. Implement PKCE + local callback server. Reference: [`packages/credentials/src/codex-oauth/flow.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/credentials/src/codex-oauth/flow.ts) — `createAuthorizationFlow` (URL with `code_challenge=S256`), `startLocalOAuthServer` (`node:http` on `127.0.0.1:1455/auth/callback`), `exchangeAuthorizationCode`.
2. Add storage helpers at `packages/harness/src/<foo>-oauth/storage.ts` that `PUT /api/config` with `{ scope: "global", key: "<foo>_oauth", value: JSON.stringify(creds), isSecret: true }`. See `storeCodexOAuth` and `getValidCodexOAuth` (with auto-refresh) in [`packages/credentials/src/codex-oauth/storage.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/credentials/src/codex-oauth/storage.ts).
...
- [`packages/types/src/index.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/types/src/index.ts) — template provider enum.
...
**Key files:** `packages/credentials/src/codex-oauth/{flow.ts,storage.ts,auth-json.ts,pkce.ts,types.ts}`, `src/commands/codex-login.ts`, `docker-entrypoint.sh` L13–71, adapter fallback `codex-adapter.ts` L810–844.

The shape conversion (our flat `{access, refresh, expires, accountId}` → Codex CLI's `{auth_mode: "chatgpt", tokens: {...}}`) lives in `credentialsToAuthJson()` at [`packages/credentials/src/codex-oauth/auth-json.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/credentials/src/codex-oauth/auth-json.ts) L37–49.
...
| Concern | File(s) |
|---|---|
| Adapter implementation | `packages/harness/src/<foo>-adapter.ts` |
| Factory registration | `packages/harness/src/index.ts` |
| Types / enums | `packages/types/src/index.ts` |
| Prompts (optional) | `src/prompts/<foo>/` |
| OAuth (optional) | `packages/harness/src/<foo>-oauth/*`, `src/commands/<foo>-login.ts` |
...
| Hooks (optional) | `src/hooks/*`, `packages/harness/src/<foo>-swarm-events.ts` |
| Skills resolver (if provider lacks native support) | `packages/harness/src/<foo>-skill-resolver.ts` |
| Models / pricing (optional) | `packages/harness/src/<foo>-models.ts` |
| Tests | `src/tests/<foo>-*.test.ts` |
| Integrations UI (optional) | `apps/ui/src/lib/integrations-catalog.ts` |
| Docs | `CLAUDE.md`, `README.md`, this guide, [Harness Configuration](/docs/guides/harness-configuration) |

**claude-managed reference files:** `packages/harness/src/claude-managed-adapter.ts`, `packages/harness/src/claude-managed-swarm-events.ts`, `packages/harness/src/claude-managed-models.ts`, `src/commands/claude-managed-setup.ts`, `apps/ui/src/lib/integrations-catalog.ts`.
...
The Anthropic Beta SDK has a few non-obvious surface differences from the conventional Anthropic `messages.create` API. The header comments in [`packages/harness/src/claude-managed-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/claude-managed-adapter.ts) (top-of-file block, ~L13–35) document them in detail, but the highlights for anyone reading the code:
...
- [`packages/harness/src/types.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/types.ts) — canonical interface definitions.
- [`packages/harness/src/codex-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/packages/harness/src/codex-adapter.ts) — the most feature-complete reference (OAuth + skills resolver + hooks + model resolver).
