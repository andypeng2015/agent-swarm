...
| [`src/providers/acp-adapter.ts`](https://github.com/desplega-ai/agent-swarm/blob/main/src/providers/acp-adapter.ts) | `Bun.spawn` of ACP target binary; bidirectional stdio JSON-RPC via `@agentclientprotocol/sdk` `ClientSideConnection` | Target-dependent — `GOOGLE_API_KEY`/`GEMINI_API_KEY` (gemini), `CLAUDE_CODE_OAUTH_TOKEN`/`ANTHROPIC_API_KEY` (claude-agent-acp), `OPENAI_API_KEY` (codex-acp), operator-supplied (custom) |
