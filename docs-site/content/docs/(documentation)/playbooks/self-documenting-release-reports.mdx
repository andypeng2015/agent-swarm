...
<JsonLd
  data={{
    ...
    step: [
      { "@type": "HowToStep", name: "Detect change", text: "Daily check: did any PRs merge in the last 24h? If not, no-op." },
      { "@type": "HowToStep", name: "Update docs", text: "When work shipped, update the docs site, CHANGELOG, and last-updated timestamps, and ride the semver bump in the same daily docs PR." },
      { "@type": "HowToStep", name: "Cut release", text: "On the next daily run, cut a GitHub release if the version already on main still lacks a tag." },
...
<Mermaid chart={`flowchart LR
  C["Cron: daily"] --> Q{"PRs merged<br/>in last 24h?"}
  Q -- no --> NO["No-op (skip)"]
  Q -- yes --> UD["Update docs +<br/>CHANGELOG + version bump PR"]
  UD --> NR["Next daily run:<br/>cut release if tag missing"]
...
- **Daily** — checks if anything actually shipped in the last 24h. If yes: update the docs site, CHANGELOG, "last updated" timestamps, and include the semver bump in the same docs PR. The GitHub release is cut by the next daily run once that bumped version is on `main`. **If nothing shipped, no-op.**
...
- **[Coder](https://templates.agent-swarm.dev/official/coder)** — docs PRs, MDX writes, CHANGELOG bumps, release tagging.
...
- **`daily-docs-update`** — daily. Checks if any PRs merged in the last 24h. **No-op if none.** Otherwise updates docs + CHANGELOG + timestamps, bumps the version in the same PR, and leaves the GitHub release to the next run once that version is on `main`. See the [No-op pattern](/docs/playbooks/patterns/no-op-workflows).
