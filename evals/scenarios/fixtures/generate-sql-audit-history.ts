 ...
 *
 * Emits `sql-audit-history.sql`: the full base dev-DB dump (`seeded-history.sql`
 * — schema + `_migrations` + minimal seed data) with the single "flux capacitor"
 * task REMOVED and a richer audit dataset of `agent_tasks` rows appended. The
 * worker under test audits that history through the swarm API and answers three
 * graded questions whose answer key lives ONLY in the seeded rows (never in the
 * task prompt):
 *
...
const BASE = new URL("./seeded-history.sql", import.meta.url);
...
// ---- compose the fixture ----
const base = await Bun.file(BASE).text();

// Drop the single flux-capacitor row from the base dump so the audit dataset is
// the ONLY agent_tasks history (the count Q1 must reflect exactly the seeded
// audit rows). Match the whole INSERT line.
const lines = base.split("\n");
const kept = lines.filter((l) => !l.includes("Calibrate the flux capacitor"));
if (kept.length === lines.length) {
  throw new Error("expected to remove the flux-capacitor row from the base dump but found none");
}

const auditBlock = [
  "",
  "-- ==== sql-audit dataset (v8.0 round-11) — appended by generate-sql-audit-history.ts ====",
  "-- Reference history only; every row is terminal (completed/failed/cancelled).",
  ...TASKS.map(insert),
  "-- ==== end sql-audit dataset ====",
  "",
].join("\n");
