...
### Changed
- **Claude Bridge toggle for the Claude harness** (#664) — `SWARM_USE_CLAUDE_BRIDGE=true|1` now routes spawned Claude sessions through the installed `claude-bridge` binary from pinned package `@desplega.ai/claude-bridge@0.1.7`, a Desplega-owned `claude -p` drop-in that drives interactive Claude Code through `tmux` for subscription-pool runs. The env is reloadable via `swarm_config`; `false|0|unset` keeps the normal `CLAUDE_BINARY` path.
