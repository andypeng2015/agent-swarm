...
import { getContextWindowSize } from "@swarm/core-utils/context-window";
...
import { interpolate } from "@swarm/core-utils/template";
import { detectVcsProvider } from "@swarm/core-utils/vcs/index";
import { authJsonToCredentialSelection } from "@swarm/credentials/codex-oauth/auth-json";
import { materializeCodexAuthJson } from "@swarm/credentials/codex-oauth/auth-json-fs";
import { loadAllCodexOAuthSlots } from "@swarm/credentials/codex-oauth/storage";
import { type CredentialSelection, resolveCredentialPools } from "@swarm/credentials/credentials";
...
} from "@swarm/otel/error-tracker";
import { initTelemetry, telemetry } from "@swarm/otel/telemetry";
import { type BasePromptArgs, getBasePrompt } from "@swarm/prompt-templates/base-prompt";
import {
...
import { configureHttpResolver, resolveTemplateAsync } from "@swarm/prompt-templates/resolver";
import {
  type ProviderName,
  type RepoGuidelines,
...
async function fetchResolvedEnv(
  ...
  const credentialSelections = await resolveCredentialPools(env, {
    ...
    // container. See `PROVIDER_CREDENTIAL_VARS` in @swarm/credentials.
    //
