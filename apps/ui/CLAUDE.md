...
|---|---|
| `bun install` | Install dependencies from the repo root workspace |
| `bun dev` | Dev server on http://localhost:5274 |
| `bun run build` | Production build |
| `bun preview` | Preview production build |
| `bun lint` / `bun lint:fix` | Biome check / auto-fix |
...
- Validate with `bun test src/tests/ui-logs-parser.test.ts`, `cd ui && bunx tsc -b`, and `cd ui && bun lint`. When touching OpenCode or Pi behavior, also run the parser against a real exported fixture and confirm zero unexpected `unknown` rows, no rendered `server.*` events, and no orphaned tool pairs.
