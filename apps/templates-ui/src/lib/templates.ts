...
import { ASSET_CATEGORIES } from "@swarm/types";
...
function getTemplatesDir(): string {
  const packageRoot = path.dirname(require.resolve("@swarm/swarm-templates/package.json"));
  const packagePath = path.join(packageRoot, "src");

  if (fs.existsSync(packagePath)) return packagePath;

  throw new Error("Templates directory not found. Expected @swarm/swarm-templates/src");
}
...
export function getTemplateFiles(category: string, name: string): TemplateResponse["files"] {
  ...
  return {
    ...
    heartbeatMd: config.files.heartbeatMd
      ? readFileOrEmpty(path.join(dir, config.files.heartbeatMd))
      : "",
