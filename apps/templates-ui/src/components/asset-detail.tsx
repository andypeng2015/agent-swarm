...
import type { AgentAssetKind, AgentAssetResponse } from "@swarm/types";
import {
  BadgeCheck,
  Calendar,
  Check,
...
import { type WorkflowDefinitionLike, WorkflowGraph } from "@/components/workflow-graph";
...
export function AssetDetail({ asset, category, name }: AssetDetailProps) {
  ...
  const { definition, prettyJson, rest } = useMemo(
    () =>
      isWorkflow
        ? extractWorkflowJson(asset.body)
        : { definition: null, prettyJson: null, rest: asset.body },
      ...
      {isWorkflow && definition && (
        <div>
          <div className="mb-4 flex items-center justify-between gap-3">
            ...
            {prettyJson && <CopyButton text={prettyJson} label="Copy workflow JSON" />}
          </div>
