...
**🔴 PIN `config.agentId` IF THE TEMPLATE IS CLAUDE-SHAPED.** When the node's `template` involves multi-tool reasoning, `bun`/`gh`/`docker` calls, structured `outputSchema`, or Slack-aware output relay, you MUST pin `config.agentId` to a worker in your swarm that is known to run an appropriate provider. Keep your deployment's agent roster in local config or notes; do not copy another swarm's worker UUIDs into this template.
...
**🔴 ALSO avoid pinning an `outputSchema`-bearing node to providers that cannot reliably complete structured-output tasks.** Some provider/worker combinations intermittently fail with `"Structured output required by outputSchema but not provided via store-progress"` because the worker does not reliably surface the `workflow-structured-output` skill at task start. For any node with `outputSchema`, pin to a worker/provider combination your deployment has verified for structured outputs; never leave it to the pool.
...
**Real examples:**
1. **2026-05-19 08:20 UTC, workflow `33d00f44-...` blog pipeline.** Two worker tasks both spawned from the same node, both received a `<sibling_tasks_in_progress>` preamble saying "user has submitted new input while sibling were running," and both insta-aborted with `progress: null`. Worker never started executing. Lead audit on 2026-05-20 found these clustered with the same shape — see shared memory `sibling-tasks-in-progress-pattern-2026-05-20`.
...
| `agent-task` step fails with `"opencode session error"` or `"Codex Exec exited"` within <15s of dispatch | Node's `config.agentId` is unpinned OR pointing at a worker/provider that cannot run the template shape | Pin `config.agentId` to a verified worker for that template shape. See §3.2. Pass FULL config (§4). |
...
- [ ] **Every `agent-task` node has `config.agentId` set** when the template requires a specific provider/worker capability. Pool-routing is the default whenever `agentId` is absent; that's how nodes race incompatible workers. See §3.2.
