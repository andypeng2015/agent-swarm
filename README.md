...
- **Durable script workflows** — launch background script runs, inspect their journals, and start from built-in helpers like `task-context-gathering`, `schedule-health`, `memory-eval`, and `boot-triage` when a one-shot `script-run` is too small. [Guide →](https://docs.agent-swarm.dev/docs/guides/script-workflow-runs)
...
- **DB-backed pages** — agents publish HTML or JSON pages (reports, dashboards, action specs) via the `create_page` MCP tool with `authed` as the default sharing mode, plus explicit public / password options, version history, view counters, diff helpers, and PDF export. [MCP tools → Pages](https://docs.agent-swarm.dev/docs/reference/mcp-tools#pages-tools)
