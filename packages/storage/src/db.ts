...
} from "@swarm/core-utils/crypto/index";
...
import { deriveProviderFromKeyType } from "@swarm/credentials/credentials";
import { parseProviderMeta } from "@swarm/credentials/provider-metadata";
import type { RateLimitWindowTelemetry } from "@swarm/otel/error-tracker";
import { telemetry } from "@swarm/otel/telemetry";
import { configureDbResolver } from "@swarm/prompt-templates/resolver";
import type {
  ActiveSession,
  Agent,
  AgentCredStatus,
...
import { getCurrentRequestUserId } from "./request-auth-context";
import { seedDefaultTemplates } from "./seed-prompt-templates";
import { emitTaskLifecycleEvent, emitTaskStarted } from "./task-lifecycle-events";
 ...
 * in `@swarm/types`. SQL strings can't import a TS const — if you add a
...
export function completeTask(
  ...
  if (row && oldTask) {
    ...
    if (options.emitLifecycleEvent !== false) {
      emitTaskLifecycleEvent("task.completed", {
        taskId: id,
        output,
        agentId: row.agentId,
...
export function failTask(id: string, reason: string): AgentTask | null {
  ...
  if (row && oldTask) {
    ...
    emitTaskLifecycleEvent("task.failed", {
      taskId: id,
...
export function cancelTask(id: string, reason?: string): AgentTask | null {
  ...
  if (row && oldTask) {
    ...
    emitTaskLifecycleEvent("task.cancelled", {
      taskId: id,
      agentId: row.agentId,
      workflowRunId: row.workflowRunId,
      workflowRunStepId: row.workflowRunStepId,
...
export function supersedeTask(
  ...
  if (row && oldTask) {
    ...
    emitTaskLifecycleEvent("task.superseded", {
      taskId: id,
      reason: args.reason,
...
export function updateTaskProgress(id: string, progress: string): AgentTask | null {
  ...
  if (row) {
    ...
    emitTaskLifecycleEvent("task.progress", {
      taskId: id,
...
export function findRecentSimilarTasks(opts: {
  ...
  // Keep in lock-step with `TERMINAL_TASK_STATUSES` in @swarm/types.
...
export function createTaskExtended(task: string, options?: CreateTaskOptions): AgentTask {
  ...
  emitTaskLifecycleEvent("task.created", {
    taskId: row.id,
    task: row.task,
    source: row.source,
    tags: options?.tags ?? [],
...
// Default markdown template generators moved to @swarm/prompt-templates/defaults
...
} from "@swarm/prompt-templates/defaults";
...
type ContextSnapshotRow = {
  ...
  // Migration 063 — see ContextFormulaSchema in @swarm/types for the value set.
...
export function recordKeyUsage(
  ...
  // keyType — see deriveProviderFromKeyType in @swarm/credentials.
 ...
 * (`packages/harness/src/swarm-events-shared.ts:48-49`) plus margin for missed
