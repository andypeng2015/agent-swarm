...
import { emitTaskLifecycleEvent } from "./task-lifecycle-events";
...
export function emitBudgetRefusalSideEffects(ctx: BudgetRefusalContext, inserted: boolean): void {
  ...
  // 2. Workflow event emit — every refusal, not just the first per day.
  emitTaskLifecycleEvent("task.budget_refused", {
    taskId: ctx.task.id,
    agentId: ctx.agentId,
    cause: ctx.cause,
