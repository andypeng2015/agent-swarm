...
import { getPage } from "@swarm/storage/db";
import { extractAndVerifyCookie } from "@swarm/storage/page-session";
import { route } from "./route-def";
import { jsonError } from "./utils";

/**
 * `/@swarm/api/*` proxy. Cookie-gated equivalent of the artifact-sdk proxy
 * (`@swarm/artifacts/server`), but lives on the MAIN API server and
