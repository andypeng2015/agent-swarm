...
import { initAgentMail } from "@swarm/integrations/agentmail/index";
import { initGitHub } from "@swarm/integrations/github/index";
import { initGitLab } from "@swarm/integrations/gitlab/index";
import { initJira } from "@swarm/integrations/jira/index";
import { initLinear } from "@swarm/integrations/linear/index";
import { startSlackApp, stopSlackApp } from "@swarm/integrations/slack/index";
import {
  initOtel,
...
import { getServerSessionsProcessed } from "@swarm/otel/server-runtime-counters";
import { initTelemetry, telemetry } from "@swarm/otel/telemetry";
import { closeDb, getSwarmConfigs, upsertSwarmConfig } from "@swarm/storage/db";
...
async function shutdown() {
  ...
  if (hasCapability("scheduling")) {
    const { stopScheduler } = await import("@swarm/workflows/scheduler");
    stopScheduler();
  }
  ...
  if (process.env.OAUTH_KEEPALIVE_DISABLE !== "true") {
    const { stopOAuthKeepalive } = await import("@swarm/integrations/oauth/keepalive");
...
try {
  const { runAllSeeders } = await import("@swarm/storage/seed");
...
httpServer
  .listen(port, async () => {
    ...
    if (hasCapability("scheduling")) {
      const { startScheduler } = await import("@swarm/workflows/scheduler");
    ...
    if (process.env.OAUTH_KEEPALIVE_DISABLE !== "true") {
      const { startOAuthKeepalive } = await import("@swarm/integrations/oauth/keepalive");
      startOAuthKeepalive();
    }
    ...
    import("@swarm/storage/memory/boot-reembed")
    ...
    import("@swarm/storage/scripts/boot-reembed")
    ...
    import("@swarm/storage/boot-scrub-logs")
