...
import { isJiraEnabled } from "@swarm/integrations/jira/app";
import { clearJiraMetadata, getJiraMetadata } from "@swarm/integrations/jira/metadata";
import { getJiraAuthorizationUrl, handleJiraCallback } from "@swarm/integrations/jira/oauth";
import { handleJiraWebhook } from "@swarm/integrations/jira/webhook";
import { deleteJiraWebhook, registerJiraWebhook } from "@swarm/integrations/jira/webhook-lifecycle";
import { ensureTokenOrThrow } from "@swarm/integrations/oauth/ensure-token";
import { deleteOAuthTokens, getOAuthTokens } from "@swarm/storage/db-queries/oauth";
import { z } from "zod";
...
function getRedirectUri(req: IncomingMessage): string {
  // Mirror Jira app startup: prefer the explicit JIRA_REDIRECT_URI override,
