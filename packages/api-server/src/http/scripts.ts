...
import { scrubObject, scrubSecrets } from "@swarm/core-utils/secret-scrubber";
import { extractScriptSignature } from "@swarm/scripts/extract-signature";
import { runScript } from "@swarm/scripts/loader";
import { resolveHttpAuditUserId } from "@swarm/storage/audit-user";
import { getAgentById, recordInlineScriptRun, upsertKv } from "@swarm/storage/db";
import { createEvent } from "@swarm/storage/events";
...
import { reembedAllScripts, searchScripts } from "@swarm/storage/scripts/embeddings";
import { extractArgsJsonSchema } from "@swarm/storage/scripts/extract-schema";
import {
  SCRIPT_SDK_TYPES,
  SCRIPT_STDLIB_TYPES,
  typecheckScript,
} from "@swarm/storage/scripts/typecheck";
import {
  type ScriptDetail,
  ScriptFsModeSchema,
  type ScriptListItem,
...
const reembedRoute = route({
  method: "post",
  path: "/api/scripts/reembed",
  pattern: ["api", "scripts", "reembed"],
  operationId: "scripts_reembed",
  summary: "Rebuild reusable script embeddings",
  description:
    "API-key authenticated maintenance route. Recomputes embeddings for all non-scratch scripts.",
  tags: ["Scripts"],
  responses: {
    200: { description: "Script embeddings rebuilt" },
  },
});
...
export async function handleScripts(
  ...
  if (reembedRoute.match(req.method, pathSegments)) {
    const reembedded = await reembedAllScripts();
    json(res, { reembedded });
    return true;
  }
