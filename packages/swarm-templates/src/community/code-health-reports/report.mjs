...
const summaryStats = Object.fromEntries(
  summaryRows.map((row) => [pick(row, ["statistic", "name"]), num(pick(row, ["value", "n"]))]),
);
const authorCountByFile = new Map();
for (const row of authorsRows) {
  authorCountByFile.set(
    normalize(pick(row, ["entity", "module", "file"])),
    num(pick(row, ["n-authors", "authors", "author-count", "count"])),
  );
}
...
fs.writeFileSync(
  path.join(outDir, "latest-pointer.json"),
  JSON.stringify(
    {
      repoName,
      runDate,
...
const rows = (items, columns) =>
  items
    .map(
      (item) =>
        `<tr>${columns.map((col) => `<td>${esc(typeof col === "function" ? col(item) : item[col])}</td>`).join("")}</tr>`,
    )
    .join("\n");
