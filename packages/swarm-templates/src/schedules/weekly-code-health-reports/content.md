...
This schedule is the templates gallery entry point for the community package in `packages/swarm-templates/src/community/code-health-reports/`. The package itself is intentionally not an agent-template config; it is a runnable playbook bundle (`PLAYBOOK.md`, `run.sh`, `report.mjs`, and `lead-prompt.md`) used by this schedule.
...
1. Use the community template in `packages/swarm-templates/src/community/code-health-reports/`. See the docs playbook at `https://docs.agent-swarm.dev/docs/playbooks/code-health-reports` for the full setup and attribution notes.
