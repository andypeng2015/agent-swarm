 ...
 *
 * Lives in `@swarm/harness` because the predicates value-import worker-harness
 ...
 * endpoint (see `AgentCredStatusSchema` in `@swarm/types`). The API never
 * runs the predicate itself — it just reads the agent row.
 */

import { scrubSecrets } from "@swarm/core-utils/secret-scrubber";
import type { AgentCredStatus, AgentLatestModel, ProviderName } from "@swarm/types";
...
export async function checkProviderCredentials(
  ...
  switch (provider) {
    ...
    case "pi": {
      const { checkPiMonoCredentials } = await import("./pi-mono-adapter");
 ...
 * expires, accountId}`) — see `@swarm/credentials/codex-oauth/types`. Returns
...
// code itself. See `AgentCredStatusSchema` in `@swarm/types` for the contract.
