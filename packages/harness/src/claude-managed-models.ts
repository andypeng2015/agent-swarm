 ...
 * `packages/harness/src/codex-models.ts`. Rates are USD per million tokens (Mtok)
    ...
    if (!warnedUnknownModels.has(model)) {
      ...
      console.warn(
        `[claude-managed-models] Unknown model "${model}" — returning $0 cost. ` +
          `Add it to CLAUDE_MANAGED_MODEL_PRICING in packages/harness/src/claude-managed-models.ts.`,
      );
    }
    return 0;
  }
