...
- **Fallback path**: `packages/ai-pricing/src/modelsdev-cache.json`
- **UI compatibility path**: `ui/src/lib/agent-runtime-models.ts` imports
  `@swarm/ai-pricing/modelsdev-cache.json` through the workspace package.
- **Loaded by**: `@swarm/ai-pricing/modelsdev-cache` + `@swarm/ai-pricing/seed-pricing`
  through the API-owned `src/be/seed-pricing.ts` wrapper, called from
  `src/server.ts` after `initDb`.
...
- **Snapshot refresh procedure**:
  ...
  - Commit the regenerated `packages/ai-pricing/src/modelsdev-cache.json` together with a bump
...
`MANUAL_PRICING_OVERRIDES` inside `@swarm/ai-pricing/seed-pricing`:
...
`packages/ai-pricing/src/modelsdev-cache.json`; otherwise add a manual override
row via the existing admin route `POST /api/pricing`.
