...
import { z } from "zod";
import { type ResolvedCredential, resolveCredential } from "./credentials";
import { parseModelStr } from "./models";
 ...
 * pattern from the provider session wrapper's previous shellout path.
 */
...
export async function defaultSpawnClaudeCli(
  ...
  // `claude -p` shellout in the LLM rater client.
