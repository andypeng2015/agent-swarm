 ...
 *
 * This module is imported from worker-side hook/provider paths so it MUST NOT
 * touch SQLite or the API-owned database layer. The boundary check enforces it.
 */
import { z } from "zod";
...
/**
 * Base prompt for session summarization. Extracted from the stop-hook path
