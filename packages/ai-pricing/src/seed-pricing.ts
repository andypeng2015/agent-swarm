/**
 * Pure projection helpers for cost-tracking seed rows.
 *
 * The vendored models.dev snapshot at `packages/ai-pricing/src/modelsdev-cache.json`
 * is the cold-start fallback for per-token rates. Runtime freshness is owned by
 * the API-side `src/be/pricing-refresh.ts`, which fetches models.dev after boot
 * and inserts newer effective rows when prices change. We project both sources
 * into rows keyed by `(provider, model, token_class)` so the recompute path in
...
import type { ModelsDevCache, ModelsDevCostBlock } from "./modelsdev-cache";
 ...
 */
export const MANUAL_PRICING_OVERRIDES: Array<{
...
export function buildModelsDevSeedRows(cache: ModelsDevCache): PricingSeedRow[] {
  ...
  for (const [id, model] of Object.entries(openrouter)) {
    ...
    // (see packages/harness/src/pi-mono-adapter.ts). Without this projection, pi runs
...
export function buildManualPricingSeedRows(): PricingSeedRow[] {
  return MANUAL_PRICING_OVERRIDES.map((o) => ({
    provider: o.provider,
    model: o.model,
...
export function buildPricingSeedRows(cache: ModelsDevCache | null): PricingSeedRow[] {
  ...
  return [...modelsdevRows, ...buildManualPricingSeedRows()];
}
