...
describe("runner requester profile prompt", () => {
  test("omits requester profile when no role or preferences are set", () => {
    expect(buildRequesterProfilePrompt({ name: "Taras", email: "t@example.com" })).toBe("");
  });

  test("formats requester role and free-text preferences", () => {
    const prompt = buildRequesterProfilePrompt({
      name: "Taras",
      email: "t@example.com",
      role: "CEO",
    ...
    expect(prompt).toContain("Their stated preferences for how you should respond and act:");
