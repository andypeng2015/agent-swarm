/**
 * Unit tests for DevinAdapter / DevinSession (`packages/harness/src/devin-adapter.ts`).
 *
...
let DevinAdapter: typeof import("@swarm/harness/devin-adapter").DevinAdapter;
...
beforeAll(async () => {
  ...
  const mod = await import("@swarm/harness/devin-adapter");
...
async function runUntilSettled(
  ...
  result: Awaited<ReturnType<import("@swarm/harness/types").ProviderSession["waitForCompletion"]>>;
}> {
