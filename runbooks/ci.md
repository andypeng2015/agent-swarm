...
| `docker-and-deploy.yml` | Push → `main` | Build images, publish release E2B templates, deploy, and publish npm/GitHub releases (only when `apps/cli/package.json` `version` changed). Not part of PR gate — see [release.md](./release.md). |
...
|---|---|---|
| **Lint and Type Check** | `bun run lint && bun run tsc:check && bun run boundary` | Worker code imported `bun:sqlite`, `@swarm/storage`, `@swarm/workflows`, or legacy `src/be` — boundary violation |
...
| **OpenAPI Spec Freshness** | `bun run docs:openapi` (must produce zero diff in `openapi.json` AND `docs-site/content/docs/api-reference/`) | Edited an HTTP route or bumped `apps/cli/package.json` `version` without regenerating |
| **Raw matchRoute check** | `! grep -rn 'matchRoute(' packages/api-server/src/http/ --include='*.ts' \| grep -v 'route-def.ts' \| grep -v 'utils.ts'` | Used `matchRoute` directly instead of the `route()` factory |
...
### When `apps/ui/` changed

| Job | Local equivalent |
|---|---|
| **UI Lint and Type Check** | `bun install --frozen-lockfile && cd apps/ui && bun run check:tokens && bun run lint && bunx tsc -b` |
...
# UI (only if you touched apps/ui/)
( cd apps/ui && bun run check:tokens && bun run lint && bunx tsc -b )
```

## Why CI fails (in order of frequency)

1. **OpenAPI drift.** You touched a route or bumped `version` in `apps/cli/package.json` and forgot `bun run docs:openapi`. Both `openapi.json` AND `docs-site/content/docs/api-reference/**` need to be committed.
...
4. **DB boundary violation.** DB-free code (`apps/cli/`, `packages/harness/`, `packages/scripts/`, and other worker/shared packages) imported from `@swarm/storage`, `@swarm/workflows`, legacy `src/be`, or `bun:sqlite`. `scripts/check-db-boundary.sh` is the fast grep check; `.dependency-cruiser.cjs` is the graph check. See root CLAUDE.md "Architecture invariants".
5. **Raw `matchRoute()`.** Use the `route()` factory in `packages/api-server/src/http/route-def.ts`.
...
CI uses `bun install --frozen-lockfile`. This means:

- **Adding/upgrading a dep:** run `bun install <pkg>`, then commit BOTH `package.json` AND `bun.lock`.
