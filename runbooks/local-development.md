...
| `SCRIPTS_ONLY_MCP` | unset (full surface) | Set `=true` on the API **and** agent containers to trim the external MCP surface to the 8 script tools ("code-mode") — all other swarm ops go through `script-run` + the scripts SDK. See [docs-site guide "Scripts-only mode"](../docs-site/content/docs/(documentation)/guides/scripts-only-mode.mdx). |
